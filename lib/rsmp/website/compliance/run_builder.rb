    ...
    module Compliance
      ...
      class RunBuilder
        ...
        def normalized_cells(cells)
          ...
          cells.map { |report| cell_summary(report) }.sort_by { |cell| [cell['core'].to_s, sxls_sort_value(cell)] }
        end
        ...
        def cell_matches?(report, expected_cell)
          expected_cell.all? { |key, value| matrix_value(report, key) == value }
        end

        def matrix_value(report, key)
          key == 'sxls' ? report_sxls(report) : report.dig('matrix', key)
        end
        ...
        def cell_summary(report)
          ...
          sxls = report_sxls(report)
          summary['sxls'] = sxls unless sxls.empty?
          summary
        end

        def report_sxls(report)
          normalize_sxls(report.dig('matrix', 'sxls'))
        end
        ...
        def sxls_sort_value(cell)
          normalize_sxls(cell['sxls']).map { |name, version| "#{name}:#{version}" }.join(',')
        end
